...
| 16 | When a plan has more than one step, plan review flags any step that's a single click or keystroke: goals starting with type, enter, press, or scroll, or clicking a button, box, or field | In live run 16, a 7B planner with a brief still split a Wikipedia search into typing, clicking the search button, and opening the article. The button step failed its own check, and the run spent its budget there | Prompt changes alone (runs 7 and 16 show a 7B planner ignores them); merging steps in code (code can't write the merged goal) |
